...
foreach ($slug in @('approach','team','invest','contact','disclaimer','subscribe','founder-led-advantage')) {
	...
	$p = $p.Replace('"contact.html"','"/contact/"')
...
$qu = $qu.Replace('"contact.html"','"/contact/"')
...
$f = $f.Replace('"contact.html"','"/contact/"')
...
$i = $i.Replace('"contact.html"','"/contact/"')
      ...
      <header class="essay-header">
        <div class="container essay-container">
          ...
          <div class="essay-meta">
            <?php /* WRITER, not the category. The sub-category (Founder in Focus etc.) led this line until 2026-09-09; the writer's name replaced it. The category is still carried by the Insights listing cards, so nothing is lost, and the byline is what a reader wants at the top of an article. Guarded on a non-empty name so a post with no resolvable author drops the slot instead of printing an empty span and a dangling dot. This used to appear LATER in the line and only on 'articles'; it now leads on every post type, because it is occupying the slot the category held everywhere. The "Written by" block in .essay-foot still repeats it, deliberately. */ $eauthor = get_the_author(); if ( $eauthor ) : ?><span><?php echo esc_html( $eauthor ); ?></span><span class="essay-meta-dot">&middot;</span><?php endif; ?>
...
$s = $s.Replace('"contact.html"','"/contact/"')
